...
- In the `file-transmission` repository, execute the below to start the application with required configuration:
    ```
    sbt "run 9575 -DcallbackFilter.allowedProtocols="http,https" -DmdgEndpoint="http://localhost:9576/mdg-stub/request" -DuserAgentFilter.allowedUserAgents="file-transmission-acceptance-tests""
    ```
